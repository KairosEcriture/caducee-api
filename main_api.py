# =============================================================================
#  CADUCEE - BACKEND API
#  Version : 3.2 (Intégration de la Géolocalisation Google Maps)
#  Date : 10/09/2025
# =============================================================================
import os; import json; import google.generativeai as genai; import googlemaps
...
# --- 1. CONFIGURATION ---
app = FastAPI(title="Caducée API", version="3.2.0")
...
# NOUVEAU : Configuration de Google Maps
...
# --- 4. ENDPOINTS API ---
@app.get("/", tags=["Status"])
def read_root(): return {"status": "Caducée API v3.2 (Géolocalisation) est en ligne."}

@app.post("/analysis", ...) # Inchangé
@app.post("/analysis/refine", ...) # Inchangé

# --- NOUVEL ENDPOINT DE GÉOLOCALISATION ---
...
def find_nearby_doctors(request: NearbyDoctorsRequest):
    ...
    try:
        ...
        doctors = []
        for place in places_result.get('results', [])[:3]: # On prend les 3 premiers
        ...
        raise HTTPException(status_code=503, detail=f"Erreur du service de géolocalisation: {e}")